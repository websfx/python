...
def getToken(url, header, username, password):
    ...
    request = requests.post(url=url, headers=header, data=json.dumps(data),verify=False)
...
def getSla(url, header, token,timeStamp2,timeStamp1):
    ...
    sla = requests.post(url=url, headers=header, data=json.dumps(data),verify=False)
    #print(type(json.loads(sla.content)["result"]))
    return json.loads(sla.content.decode())["result"]
...
with open("zyly.txt","w",encoding="utf8")  as f:
    now_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    sla = round(sla,2)
    project = "项目名称：中移路由器"
